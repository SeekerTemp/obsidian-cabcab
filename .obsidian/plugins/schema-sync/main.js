...
function renderBaseYaml(schemaName, fields, recordFolder) {
  ...
  const columns = ["file.name", ...bound.map(([name]) => attachments.includes(name) ? `formula.${name}Image` : name)];
  return [
    "# Generated by Schema Sync. Edits are overwritten on the next sync.",
    ...(attachments.length ? ["formulas:", ...attachments.map((name) => `  ${name}Image: image(${name}.path)`)] : []),
    "views:",
    "  - type: table",
    `    name: ${schemaName}`,
    "    filters:",
