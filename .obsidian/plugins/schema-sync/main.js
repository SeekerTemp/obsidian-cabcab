...
// The single rule for which fields get a value list. Shared by the generator and
// the dashboard's open-config button so the two cannot drift apart.
function configFileNameFor(fieldName, definition) {
...
class SchemaSyncPlugin extends Plugin {
  ...
  async openFieldConfig(schemaName, fieldName) {
    ...
    if (!fileName) {
      ...
      if (definition.relation?.target) return new Notice(`"${fieldName}" is a relation to ${definition.relation.target} — its values come from that entity's records, not a config list.`);
